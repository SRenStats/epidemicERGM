...
net <- set.vertex.attribute(net, "SES", rep(0:1, each = 25))
...
target.stats <- c(50, 10, 5, 5) # correspond to formation formula above
...
status.vector <- rbinom(50, 1, 0.03)   # seed initial infected individuals
...
plot(sim1, type = "network", at = 50, col.status = TRUE,main = "Prevalence at t100")
...
colors <- ifelse(names>25,"tomato","steelblue")
